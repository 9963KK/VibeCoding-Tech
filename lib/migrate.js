...
async function detectVersion(projectDir) {
  ...
  if (await fs.pathExists(docsDir)) {
    ...
    const legacyDocs = ['规范文档.md', '项目文档.md', '功能清单.md', '附加材料.md'];
...
async function checkLegacyHooks(hooksDir) {
  ...
  for (const hookFile of hookFiles) {
    ...
    if (await fs.pathExists(hookPath)) {
      ...
      // 检查旧版本的路径问题
      if (content.includes('docs/功能清单.md') && !content.includes('docs/core/功能清单.md')) {
...
async function getMigrationPlan(projectDir, versionInfo) {
  ...
  const legacyDocs = ['规范文档.md', '项目文档.md', '功能清单.md', '附加材料.md'];
  for (const doc of legacyDocs) {
    const legacyPath = path.join(docsDir, doc);
    const newPath = path.join(coreDir, doc);

    if (await fs.pathExists(legacyPath) && !await fs.pathExists(newPath)) {
      plan.details.docsToMove.push({ from: doc, to: `core/${doc}` });
      plan.tasks.push(`迁移文档: docs/${doc} → docs/core/${doc}`);
    }
  }
