...
1. 读取 `.claude/settings.json`（如存在）获取当前版本（仅参考）
2. 读取 `.opencode/opencode.jsonc`（如存在）确认 OpenCode 配置
3. 读取 `template/docs/core/*.md` 作为迁移模板
4. 读取以下文件判断当前文档结构：
...
project:
  ...
  modules: []
  paths:
    project_root: $PWD (包含 .git 或 .opencode)
